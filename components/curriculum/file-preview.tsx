...
export function FilePreview({ selection, onDelete }: FilePreviewProps) {
  ...
  const { Icon } = meta
  const fileUrl = material.storagePath ? getPublicFileUrl(material.storagePath) : null

  return (
    <div className="flex h-full flex-col">
      {/* Preview canvas */}
      <div className="flex flex-1 items-center justify-center rounded-lg border border-border bg-muted/40 p-6">
        <PreviewCanvas kind={kind} name={material.name} url={fileUrl} />
      </div>
...
function PreviewCanvas({ kind, name, url }: { kind: FileKind; name: string; url: string | null }) {
  if (kind === "image" && url) {
    return (
      // eslint-disable-next-line @next/next/no-img-element
      <img src={url} alt={name} className="max-h-64 max-w-full rounded-md object-contain shadow-sm" />
    )
  }

  if (kind === "pdf" && url) {
    return <iframe src={url} title={name} className="h-64 w-full max-w-md rounded-md border border-border bg-background" />
  }
