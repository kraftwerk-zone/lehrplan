-- Reparatur über JWT (auth.uid()) – funktioniert aus der App ohne UUID-Parameter
...
-- Aus der App aufrufbar: nutzt auth.uid() aus dem JWT
create or replace function public.repair_my_curriculum()
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  delete from public.materials where user_id = auth.uid();
  delete from public.sub_topics where user_id = auth.uid();
  delete from public.topics where user_id = auth.uid();

  perform public.seed_curriculum_for_user(auth.uid());
end;
$$;

grant execute on function public.seed_curriculum_for_user(uuid) to authenticated;
grant execute on function public.repair_my_curriculum() to authenticated;
