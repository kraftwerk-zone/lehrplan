-- Unterthemen-Seed hing an Fach-Namen (s_math, s_deutsch, …).
-- Nutzer mit abweichenden Fach-IDs bekamen nur Teilmengen (z. B. 1/7).
...
declare
  ...
  linked_sub_topics integer;

  demo_sub_topics constant integer := 7;
begin
  if p_user_id is null then
    raise exception 'p_user_id ist null';
  end if;
  ...
  select count(*)::integer into linked_sub_topics
  from public.sub_topics st
  inner join public.topics t on t.id = st.topic_id and t.user_id = p_user_id
  inner join public.subjects s on s.id = t.subject_id and s.user_id = p_user_id
  where st.user_id = p_user_id;

  if linked_topics > 0 and linked_sub_topics >= demo_sub_topics then
    return;
  end if;
  ...
  t_zahlenraum := coalesce(
    (select id from public.topics where user_id = p_user_id and id = public.scoped_id(p_user_id, 't-zahlenraum')),
    (select id from public.topics where user_id = p_user_id and name = 'Zahlenraum 100' limit 1),
    public.scoped_id(p_user_id, 't-zahlenraum')
  );
  t_addition := coalesce(
    (select id from public.topics where user_id = p_user_id and id = public.scoped_id(p_user_id, 't-addition')),
    (select id from public.topics where user_id = p_user_id and name = 'Halbschriftliche Addition' limit 1),
    public.scoped_id(p_user_id, 't-addition')
  );
  t_lesen := coalesce(
    (select id from public.topics where user_id = p_user_id and id = public.scoped_id(p_user_id, 't-lesen')),
    (select id from public.topics where user_id = p_user_id and name = 'Lesen: Ganzschrift' limit 1),
    public.scoped_id(p_user_id, 't-lesen')
  );
  t_wetter := coalesce(
    (select id from public.topics where user_id = p_user_id and id = public.scoped_id(p_user_id, 't-wetter')),
    (select id from public.topics where user_id = p_user_id and name = 'Wetter & Jahreszeiten' limit 1),
    public.scoped_id(p_user_id, 't-wetter')
  );
  t_rhythmus := coalesce(
    (select id from public.topics where user_id = p_user_id and id = public.scoped_id(p_user_id, 't-rhythmus')),
    (select id from public.topics where user_id = p_user_id and name = 'Rhythmus & Notation' limit 1),
    public.scoped_id(p_user_id, 't-rhythmus')
  );
  ...
  if exists (select 1 from public.topics where user_id = p_user_id and id = t_zahlenraum) then
    insert into public.sub_topics (
      ...
      (public.scoped_id(p_user_id, 'st-orientierung'), p_user_id, t_zahlenraum, 'Orientierung im Hunderterfeld', 3, 1, 'Zahlenreihe bis 20 als Brücke, Partnerarbeit', 'Zahlenrätsel bis 1000, freie Muster erfinden', '{}'::jsonb, 0),
      (public.scoped_id(p_user_id, 'st-buendeln'), p_user_id, t_zahlenraum, 'Bündeln & Stellenwert', 4, 2, 'Konkretes Legen mit Zehnerstangen', 'Stellenwerttabelle bis Tausender', '{}'::jsonb, 1)
    on conflict (id) do nothing;
  ...
  if exists (select 1 from public.topics where user_id = p_user_id and id = t_addition) then
    insert into public.sub_topics (
      id, user_id, topic_id, name, duration_in_days, buffer_in_days,
      differentiation_support, differentiation_challenge, points, sort_order
    ) values
      (public.scoped_id(p_user_id, 'st-add-strategien'), p_user_id, t_addition, 'Rechenstrategien entwickeln', 5, 2, 'Schrittweises Rechnen mit Zahlenstrahl', 'Eigene Strategien begründen & vergleichen', '{}'::jsonb, 0)
    on conflict (id) do nothing;
  ...
  if exists (select 1 from public.topics where user_id = p_user_id and id = t_lesen) then
    insert into public.sub_topics (
      id, user_id, topic_id, name, duration_in_days, buffer_in_days,
      differentiation_support, differentiation_challenge, points, sort_order
    ) values
      (public.scoped_id(p_user_id, 'st-einstieg-buch'), p_user_id, t_lesen, 'Einstieg & Leseerwartung', 2, 1, 'Hörbuch parallel, reduzierte Kapitel', 'Lesetagebuch mit Reflexionsfragen', '{}'::jsonb, 0),
      (public.scoped_id(p_user_id, 'st-lesetagebuch'), p_user_id, t_lesen, 'Lesetagebuch führen', 6, 2, 'Satzanfänge vorgegeben', 'Perspektivwechsel schreiben', '{}'::jsonb, 1)
    on conflict (id) do nothing;
  ...
  if exists (select 1 from public.topics where user_id = p_user_id and id = t_wetter) then
    insert into public.sub_topics (
      id, user_id, topic_id, name, duration_in_days, buffer_in_days,
      differentiation_support, differentiation_challenge, points, sort_order
    ) values
      (public.scoped_id(p_user_id, 'st-wetter-beobachten'), p_user_id, t_wetter, 'Wetter beobachten & messen', 4, 1, 'Symbolkarten für Wetterphänomene', 'Messreihe über zwei Wochen auswerten', '{}'::jsonb, 0)
    on conflict (id) do nothing;
  ...
  if exists (select 1 from public.topics where user_id = p_user_id and id = t_rhythmus) then
    insert into public.sub_topics (
      id, user_id, topic_id, name, duration_in_days, buffer_in_days,
      differentiation_support, differentiation_challenge, points, sort_order
    ) values
      (public.scoped_id(p_user_id, 'st-bodypercussion'), p_user_id, t_rhythmus, 'Bodypercussion', 2, 0, 'Feste Bewegungsmuster nachmachen', 'Eigene Rhythmen komponieren', '{}'::jsonb, 0)
    on conflict (id) do nothing;
